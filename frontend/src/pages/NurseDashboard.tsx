...
export default function NurseDashboard() {
  ...
  // Get current patient vital signs history
  const activePatientVitalsHistory = useMemo(() => {
    if (!selectedPatient) return [];
    return getVitalSignsByPatient(`patient-${selectedPatient.id}`);
  ...
  async function loadPatients() {
    try {
      ...
      console.error('Unable to load accessible patients', error);
      setPatients([]);
  ...
  async function handleVitalsSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!isOnline) {
      setNotification('⚠️ Offline. Vitals must be saved while connected to the network.');
      return;
    }
    ...
    try {
      ...
      const pastVitals = getVitalSignsByPatient(patientDbId);
      const prevVital = pastVitals.length > 0 ? pastVitals[pastVitals.length - 1] : null;
      ...
      // 2. Reactively evaluate and update patient clinical status
      ...
      // Refresh list to pull live calculations
      await loadPatients();
      
      setVitals(initialVitalsForm);
      setNotification(`✓ Vitals logged and synced for ${selectedPatient.firstName} ${selectedPatient.lastName}. Status: ${newCondition}`);
    } catch (error) {
  ...
  async function handleDownloadHandoverReport() {
    ...
    try {
      ...
      const patientDbId = `patient-${selectedPatient.id}`;
      const patientVitals = getVitalSignsByPatient(patientDbId);
...
------------------------------
${patientVitals.length === 0 ? 'No vitals signs currently logged on this shift.' : 
  patientVitals.map(v => `[${new Date(v.timestamp).toLocaleTimeString()}]
  - HR: ${v.heartRate} bpm
...
Department Console Sign-off: [ONLINE SYNCED]
      ...
      a.href = url;
      a.download = `Clinical_Handover_${selectedPatient.patientIdentifier}_${new Date().toISOString().split('T')[0]}.txt`;
