...
public interface EscalationRuleRepository extends JpaRepository<EscalationRule, String> {
    ...
    @Query("SELECT r FROM EscalationRule r WHERE r.alertType = :alertType AND r.priority = :priority AND r.active = :active")
    List<EscalationRule> findByAlertTypeAndPriorityAndActive(
        @Param("alertType") String alertType, 
        @Param("priority") EscalationRule.AlertPriority priority, 
        @Param("active") Boolean active);
    
    @Query("SELECT r FROM EscalationRule r WHERE r.alertType = :alertType AND r.active = :active")
    List<EscalationRule> findByAlertTypeAndActive(@Param("alertType") String alertType, @Param("active") Boolean active);
    
    @Query("SELECT r FROM EscalationRule r WHERE r.priority = :priority AND r.active = :active")
    ...
    @Query("SELECT r FROM EscalationRule r WHERE r.active = :active ORDER BY r.escalationLevel ASC")
    List<EscalationRule> findByActiveOrderByEscalationLevelAsc(@Param("active") Boolean active);
    
    @Query("SELECT r FROM EscalationRule r WHERE r.targetRole = :targetRole AND r.active = :active")
    List<EscalationRule> findByTargetRoleAndActive(@Param("targetRole") String targetRole, @Param("active") Boolean active);
    
    @Query("SELECT r FROM EscalationRule r WHERE r.escalationLevel = :escalationLevel AND r.active = :active")
    ...
    @Query("SELECT r FROM EscalationRule r WHERE r.timeBasedEscalation = true AND r.active = :active")
    List<EscalationRule> findByTimeBasedEscalationAndActive(@Param("active") Boolean active);
    
    @Query("SELECT r FROM EscalationRule r WHERE r.conditionBasedEscalation = true AND r.active = :active")
    List<EscalationRule> findByConditionBasedEscalationAndActive(@Param("active") Boolean active);
    
    @Query("SELECT r FROM EscalationRule r WHERE r.priorityBasedEscalation = true AND r.active = :active")
